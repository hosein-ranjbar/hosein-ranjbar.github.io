...
(function () {
  ...
  /** باز کردن لیست استان‌ها — منوها بسته؛ فقط نام استان */
  function expandProvincesList() {
    var landing = document.getElementById('landing');
    var main = document.getElementById('provinces');
    var btn = document.getElementById('expand-arrow');
    var btnS = document.getElementById('expand-arrow-sticky');
    ...
    if (btn) {
      ...
      btn.classList.remove('c-orange', 'c-white', 'c-black');
    }
    if (btnS) {
  ...
  function setupExpandArrow() {
    ...
    if (btn) {
      ...
      btn.addEventListener('click', function () {
        if (listExpanded) collapseProvincesList();
        else expandProvincesList();
      });
    }
    if (btnS) {
      btnS.addEventListener('click', function () {
        collapseProvincesList();
      });
    }
  }
  ...
  /** یک‌بار: تایپ پاساژگردی → پاک → تبلیغات */
  function runFooterTypewriter() {
    var el = document.getElementById('footer-typewriter');
    var caret = document.querySelector('.footer-caret');
    if (!el) return;
    var first = 'پاساژگردی';
    var second = 'تبلیغات';
    var i = 0;
    ...
    function typeWord(word, done) {
      ...
      function step() {
        if (i <= word.length) {
          ...
          i++;
          setTimeout(step, 90);
        } else if (done) setTimeout(done, 600);
      }
      step();
    }
    function deleteWord(done) {
      ...
      function step() {
        if (cur.length > 0) {
          ...
          el.textContent = cur;
          setTimeout(step, 55);
        } else if (done) setTimeout(done, 200);
      }
      step();
    }
    typeWord(first, function () {
      deleteWord(function () {
        typeWord(second, function () {
  ...
  function init() {
    ...
    setTimeout(runFooterTypewriter, 400);
    try {
